...
class NLSE_3d(NLSE):
    """A class to solve the 3D NLSE (spatio-temporal pulse propagation)."""

    def __init__(
        self,
        alpha: float,
        energy: float,
        window: list[float] | tuple[float, ...],
        n2: float,
        D0: float,
        vg: float,
        V: np.ndarray | None,
        ...
        energy : float
            Total energy in J.
        window : list of float or tuple of float
            Computational window in the transverse plane (index 0) in m
            and longitudinal direction (index 1) in s. Can also be
            ``[window_x, window_y, window_t]``.
        n2 : float
