...
# The one example that sets its own step, and the reason is the sampling
# rather than the propagation: the animation below wants frames evenly
# spaced in z, and sampling every save_every steps only gives that if the
# steps are all the same length. Let the solver choose and the frames land
# wherever the step happened to be, which is not a movie of the precession.
DELTA_Z = z_nl / 6
simu.L = 48 * z_nl
...
def callback_samples(sim, A, z, i):
    if i % save_every == 0:
        ...
        # and assigning one into this numpy buffer does not work.
        E_samples[i // save_every] = sim._as_host_array(A)
